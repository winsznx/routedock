...
app.use(
  ...
  routedock({
    ...
    onOrphaned: async (channelId: string, info) => {
      if (!supabase) return
      const stroopAmount = Math.round(parseFloat(info.cumulativeAmount) * 1e7).toString()
      const { error } = await supabase
        .from('sessions')
        .update({
          status: 'closing',
          cumulative_amount: stroopAmount,
          last_signature: info.lastSignature,
          voucher_count: info.voucherCount,
        })
        .eq('channel_id', channelId)
      if (error) console.error('[supabase] session orphan update failed:', error.message)
      else console.log(`[supabase] session marked closing (orphaned): ${channelId}`)
    },
