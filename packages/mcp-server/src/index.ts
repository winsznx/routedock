    ...
    switch (name) {
      case 'pay_for_data': {
        ...
        if (max_amount) {
          ...
          const manifest = await manifestResponse.json() as Record<string, { pricing?: Record<string, { amount?: string }> }>
          
          const pricing = (manifest as any).pricing?.[preferred_mode || 'x402'] || (manifest as any).pricing?.['x402']
